...
export function generateStaticParams() {
    return kanchanaburiAttractions.map((attraction) => ({
...
export default async function AttractionDetailPage(props: AttractionDetailProps) {
    ...
    const attraction = kanchanaburiAttractions.find((a) => a.slug === params.slug);
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-black font-sans selection:bg-amber-500 selection:text-white">
            {/* Minimal Nav */}
            <nav className="fixed w-full z-50 glass-nav transition-all duration-300">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-20">
                        <div className="flex items-center gap-4">
                            <Link href="/province/kanchanaburi" className="text-white hover:text-amber-400 font-bold flex items-center gap-2 transition-colors">
                                ← ย้อนกลับ
                            </Link>
                        </div>
                        <Link href="/">
                            <span className="text-2xl font-extrabold bg-clip-text text-transparent bg-linear-to-r from-amber-500 to-yellow-300 drop-shadow-md tracking-tight cursor-pointer">
                                ไปไหนดี
                            </span>
                        </Link>
                    </div>
            ...
            <div className="relative h-[60vh] w-full">
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
                    <div className="max-w-7xl mx-auto">
                        <span className="inline-block px-4 py-1 rounded-full bg-amber-600 text-white text-sm font-semibold mb-4">
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="bg-white dark:bg-zinc-900 rounded-3xl p-8 md:p-12 shadow-xl -mt-24 relative z-10">
                    <div className="prose prose-lg dark:prose-invert max-w-none">
                        <h2 className="text-2xl font-bold text-amber-600 mb-6">รายละเอียด</h2>
