...
export function generateStaticParams() {
    return phitsanulokAttractions.map((attraction) => ({
...
export default async function AttractionDetailPage(props: AttractionDetailProps) {
    ...
    const attraction = phitsanulokAttractions.find((a) => a.slug === params.slug);
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-black font-sans selection:bg-thai-gold selection:text-white">
            {/* Minimal Nav */}
            <nav className="fixed w-full z-50 glass-nav transition-all duration-300">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-20">
                        <div className="flex items-center gap-4">
                            <Link href="/province/phitsanulok" className="text-white hover:text-thai-gold font-bold flex items-center gap-2 transition-colors">
                                ← ย้อนกลับ
                            </Link>
                        </div>
                        <Link href="/">
